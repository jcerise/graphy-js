...
Template.nodes_page.events({
    'click #new_node' : function() {
        var graph = Session.get("current_graph");
        Graphy.Nodes.create("New Node", "blue", {x:250, y:250}, graph._id);
    },
    'dblclick .node_text' : function() {
        var text = prompt("Please enter new text", this.text);
        if (text) {
          Graphy.Nodes.change_text(this._id, text);
        }
    },
...
function drag_node() {
    ...
    $("#" + this._id).draggable({
        ...
        stop: function(e, u) {
            ...
            Graphy.Nodes.move(node_id, {x: u.offset.left, y: u.offset.top});
        },
        drag: node_moved
    });
};
...
function add_relationship(e) {
    if ($(e.target).hasClass("node_border")) {
        ...
        if (!node_a) {
            ...
            Graphy.Relationships.create(node_a._id, this._id);
...
function draw_line(e) {
    ...
    if (node) {
        ...
        setup_line(d3.select("#mouse_line"), n, {x: e.pageX, y: e.pageY});
    }
};
...
function color_node(e) {
    ...
    if (color && node) {
        Graphy.Nodes.change_color(node, color);
    }
}

function delete_relationship(e) {
    Graphy.Relationships.delete($(e.target).attr("rel_id"));
    render_lines();
}
